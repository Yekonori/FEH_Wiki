										...
										<div class="row">
											...
											<div class="col-md-4">
												...
												<select name="typeEffet">
													<option value="0">-- Type Effet--</option>
													<?php
														$sql = "SELECT * FROM `passifs_type_effet` ORDER BY `ID_Type_Effet :`";
														$Type_Effet = mysqli_query($link, $sql);
														$i = 0;
														while ($data = mysqli_fetch_assoc($Type_Effet)) {?>
															<option value=" <?php echo $data['ID_Type_Effet :']; ?>"> <?php echo $data['Type_Effet :']; ?></option>
															<?php $i++;
														}
													?>
												</select>
